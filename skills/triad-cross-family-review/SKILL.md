...
version: 0.36.2
# changelog:
#  0.36.2 (2026-09-18): doc-only — `references/failure-modes.md` gains the codex-leg `input-delivery-failed` (65) row: the wrapper's own stdin transport did not confirm delivery of the leg body — re-dispatch that leg ONCE on the frozen packet, a second one makes the family terminally missing (rule 13). Origin: gate r1 of the stdin fail-closed engine slice (x-claude-high finding).
#  0.36.1 (2026-09-18): H1 (plan `docs/superpowers/plans/2026-09-18-cfr-post-review-hardening.md`; owner D1) — `lib/agy_hook.py` is an ALLOW-LIST: `ALLOW_TOOLS` = the census's five review tools (t9 pins it equal to the wrapper's `AGY_REVIEW_TOOLS` by loading both artifacts at test time), EVERY other name — mutating, command, network, subagent, MCP, prompt-shaped, or one agy ships tomorrow — is DENIED before it runs (blocked, non-voiding). Reverses S2's "unknown → allow": an unknown mutating tool must be blocked before it executes, not voided after (three review lenses + three plan-gate families converged). `DENY_TOOLS`/`DENY_PREFIXES` leave the code; the 57-name registry stays in t9 as a DENY-side pin. The rendered READ-GRANT says "Tools outside the five are BLOCKED before they run" (leg-contracts mirror regenerated); the agent-file sentence ("mutating and network") is unchanged — true, narrower, DISCLOSED (changing it forces a host-wide `--setup-agents` re-run). `permissionOverrides` is never emitted. W-29 gains its § section; NEW W-30: agy is off-site only (owner), #1036 is not an in-company risk. Live proof (two agy dispatches: clean leg → HOOK_LOAD_PASS; write + interaction attempt → deny rows, finish still arrives) pending owner OK.
