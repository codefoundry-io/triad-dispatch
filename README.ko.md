...
비공개 저장소 인증은 기존 git 자격증명(`ssh-agent` 의 SSH 키, 또는 `gh` /
git credential helper)을 사용합니다. 백그라운드 자동 업데이트에는 환경변수
`GITLAB_TOKEN` / `GITHUB_TOKEN` 가 필요합니다.

**로컬 설치 (빌드한 폴더에서 직접).** 사내 저장소에 publish 하기 전에
...
`.claude/agents/` 가 이미 있는 체크아웃 말고. 플러그인 skill/agent 는
...
이 설정이 없으면 디스패치마다 프롬프트가 뜨거나(headless 환경에서는 거부).
...
- 리뷰 가치가 있거나 정확성이 중요한 작업을 머지하기 전, leader 는
  **`triad-cross-family-review`** 를 실행합니다 (self-rule #6): 서로 다른
