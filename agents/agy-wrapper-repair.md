...
**Mechanics**: Read `~/.config/triad-dispatch/classifier-patches.json` (or start from `{}` if the file is absent), add ONE entry under the failing CLI's `antigravity` envelope, then Write the merged JSON back atomically (valid JSON only — no comments, no trailing prose). The engine's two-layer `classify()` merges this extension at runtime, so the re-run with `--repair-mode` will route correctly.
